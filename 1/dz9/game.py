...
def draw_tic_tac_toe (scr, items):
    for i in range (3):
        for j in range (3):
            if items [i][j] == '0':
                pygame.draw.circle (scr, (225, 0, 0), (j * 100 + 50, i * 100 + 50), 45)
            elif items [i][j] == 'x':
                pygame.draw.line (scr, (0, 0, 225), (j * 100 + 5, i * 100 + 5), (j * 100 + 95, i * 100 + 95), 3)
                pygame.draw.line (scr, (0, 0, 225), (j * 100 + 95, i * 100 + 5), (j * 100 + 5, i * 100 + 95), 3)
...
def get_win_chek (fd, symbol):
    ...
    for line in fd:
        if line.count (symbol) == 3:
            flag_win = True
    for i in range (3):
        if fd [0][i] == fd [2][i] == fd [2][i] == symbol:
            flag_win = True
    ...
    for event in pygame.event.get ():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            ...
            if player_win or ai_win:
                game_over = True
                if player_win:
                    pygame.display.set_caption ("pobeda")
                else:
                    pygame.display.set_caption ("pobeda kompa")
            elif field [0].count ('x') + field [0].count ('0') + field [1].count ('x') + \
                field [1].count ('0') + field [2].count ('x') + field [2].count ('0') == 8:
                game_over = True
                pygame.display.set_caption ("ravni")
